...
spanTitle.innerHTML = spanTitle.innerHTML + " Leo";
...
function buildToDo(todo, index) {
  ...
  //Primera forma de añadir clases
  toDoShell.classList.add("toDoShell");
  let toDoText = document.createElement("span");
  //Segunda forma de añadir Clases o ID
  //toDoText.id = index;
  toDoText.setAttribute("id", index);
  toDoText.innerHTML = todo.description;
  if(todo.complete === true) {
    //Tercera forma de añadir Clases
...
function buildToDos(toDos) {
  // Tu código acá:
  //.map retorna un nuevo array con los argumentos(todo,index)
 let array = toDos.map(function(todo, index) {
   return buildToDo(todo, index)
 })
 return array;

//return toDos.map(buildToDo) //Hace lo mismo que lo de arriba
}
...
function displayToDos() {
  ...
  let arrayToDo = buildToDos(toDoItems);
...
//2 formas de añadirle el event click
